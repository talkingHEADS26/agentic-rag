...
- Fragen zu Angeboten, Programmen und Preisen aus einer eigenen Wissensdatenbank beantwortet (RAG)
...
Caddy (Port 443, HTTPS + WSS)
    │
    ├── /            → Express → index.html (Chat-Widget)
...
├── index.html             Chat-Widget (Test-Seite + embeddable Widget)
├── admin/upload.html      Admin-Interface für Wissensbasis-Management
├── talkingHEADS_Wissensdatenbank.md  Vollständige Wissensbasis (lokal)
...
```
ask_time → ask_week → ask_week_part → show_slots → ask_name → ask_email → ask_phone → [BUCHUNG]
```
...
Schritt 7: "Deine Handynummer?"
           → Google Calendar: Event anlegen + Bestätigungs-E-Mail
```
...
```
Du bist der KI-Assistent von TalkingHEADS. Du beantwortest ausschließlich
...
| Widget-Button kein Effekt | `<div id="th-typing">` fehlte im HTML → JS-Crash vor Listener-Registrierung | Element hinzugefügt |
...
| `invalid_client` bei OAuth | CLIENT_SECRET = CLIENT_ID | Korrektes Secret aus Google Console |
| Node 20 WebSocket-Fehler in Docker | Supabase Realtime braucht native WS | Node 22 im Dockerfile |
| SSL `ERR_CERT_COMMON_NAME_INVALID` | Browser verbindet via IPv6 `2a01:4f8:d0a:1200::2` (Hetzner Infra) statt VPS | AAAA-Record aus Hetzner DNS gelöscht |
...
## 12. Server-Management Cheat Sheet
